...
struct base_model{
    ...
    myMap forward(myMap & inputs_){
        ...
        for (auto &input:inputs_){ // check the inputs and set them to default inputs
            ...
            this->default_inputs[input.first] = input.second;
            
        }
        for (auto&[key,value]:this->default_inputs){
            this->engine->getInputVariable(key)->setValue(value);
        }
...
struct Fuzzy_IL8_IL1b:public base_model {
    ...
    void define_rules(){
        ...
        vector<string> late_diff_rules = { // these are dummy definitions. IL8 and IL1b dont regulate late diff
            "if IL8 is Neg then late_diff is M",
            "if IL8 is not Neg then late_diff is M",
            "if IL1b is Neg then late_diff is M",
            "if IL1b is not Neg then late_diff is M"
        };
        for (auto&rule:late_diff_rules){
            this->controller->addRule(Rule::parse(rule.c_str(), this->engine));
        }
...
struct Fuzzy_Mg:public base_model {
    Fuzzy_Mg(myMap &params_):base_model(params_){
        this->initialize("Fuzzy_Mg");
        this->define_antecedents();
        this->define_consequents();
        this->define_rules();
        this->reset();
    ...
    void define_rules(){
        ...
        vector<string> late_diff_rules = {
            "if Mg is Des_l then late_diff is L",
            "if Mg is not Des_l then late_diff is M"
        };
        for (auto&rule:early_diff_rules){
            this->controller->addRule(Rule::parse(rule.c_str(), this->engine));
        }
